package com.example.aop_part04_chapter04
...
import com.example.aop_part04_chapter04.databinding.ActivityMainBinding
import com.google.android.material.appbar.AppBarLayout
import kotlin.math.abs

class MainActivity : AppCompatActivity() {

    private var isGathringMotionAnimationg: Boolean = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initAppBar() // 점점 어두워지는 alpha값이 곧 0으로 됨
        initInsetMargin() // 아래에서 위로 스크롤 할때 영역밖까지 그냥 나가짐 !!
    ...
    private fun initAppBar() {
        binding.appBar.addOnOffsetChangedListener(AppBarLayout.OnOffsetChangedListener { appBarLayout, verticalOffset ->
            ...
            val percentage = realAlphaVerticalOffset / realAlphaScrollHeight // realAlphaVerticalOffset : 얼만큼 이동했는가
            binding.toolbarBackgroundView.alpha =
                1 - (if (1 - percentage * 2 < 0) 0f else 1 - percentage * 2) // percentage * 2했었을때 0보다 작으면
            // 420dp중에 300dp는 무시가 되는거고 300dp가 넘어가는 순간 alpha값에 영향을 줄수있음
        })
        initActionBar()
    }
